...
def build_specs_text(agent_sections: dict, defaults: dict) -> str:
    objective = " ".join(agent_sections.get("preamble", [])).replace("# Project Guidance", "").strip()
    ...
    spec = {
        ...
        "objective": objective or "Clean raw data, engineer member-level features, and build inpatient visit risk outputs.",
        "approval_flow": defaults["approval_flow"],
        "workflow": workflow or ["Clean raw data", "Build staged member-level data"],
        ...
        "notes": data_locations + guardrails,
    }
    yaml_lines = []
...
def create_final_data() -> None:
    ...
    run_step(["python", "skills/healthcare-data-profile-clean/scripts/profile_raw_data.py", "--input-dir", "data/raw", "--output-path", "data/staging/profile_summary.json"])
    run_step(["python", "skills/healthcare-column-standardizer/scripts/standardize_columns.py", "--input-dir", "data/raw", "--output-dir", "data/staging", "--manifest-path", "data/staging/column_standardization_manifest.json"])
    run_step(["python", "skills/data-cleaning-tests/scripts/validate_staging_tables.py", "--input-dir", "data/staging"])
    run_step(["python", "skills/inpatient-target-labeler/scripts/create_inpatient_target.py"])
    run_step(["python", "skills/member-lookback-features/scripts/build_member_lookback_features.py", "--staging-dir", "data/staging", "--target-path", "data/processed/member_inpatient_target_next_6m.csv", "--output-path", "data/processed/member_lookback_features.csv"])
    run_step(["python", "skills/tabular-preprocessing/scripts/build_preprocessing_manifest.py", "--input-path", "data/processed/member_lookback_features.csv", "--output-path", "data/processed/preprocessing_manifest.json"])
    run_step(["python", "skills/logistic-risk-model/scripts/train_logistic_model.py", "--features-path", "data/processed/member_lookback_features.csv", "--target-path", "data/processed/member_inpatient_target_next_6m.csv", "--output-dir", "data/processed/model_artifacts"])
