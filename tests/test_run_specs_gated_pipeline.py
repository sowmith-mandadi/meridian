...
def test_mark_specs_approved_updates_top_level_flag_only(tmp_path):
    specs_path = tmp_path / "tech_spcs.yml"
    specs_path.write_text(
        "\n".join(
            [
    ...
    changed = pipeline_module.mark_specs_approved(specs_path)

    assert changed is True
    assert pipeline_module.is_specs_approved(specs_path) is True
    assert 'step_3: "Human sets final_data_approved: true"' in specs_path.read_text(encoding="utf-8")
